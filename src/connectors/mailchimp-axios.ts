...
type MailchimpSubscribeStatus = "success" | "error";
...
const rawResponseToResponse = (
  rawResponse: AxiosResponse<RawMailchimpSubscribeResponse>
): AxiosResponse<MailchimpSubscribeResponse> => {
  if (
    rawResponse.data &&
...
interface MailchimpAxiosMethods {
  ...
  ) => Promise<AxiosResponse<MailchimpSubscribeResponse>>;
}

export interface MailchimpAxiosInstance
...
>({}, baseAxios, {
  subscribe: async function (data, config) {
    const _config = mergeConfig(
      this.defaults as unknown as AxiosRequestConfig<any>,
      config ?? {}
    );
