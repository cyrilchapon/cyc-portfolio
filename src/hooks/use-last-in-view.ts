...
export const useLastInView = <U extends string>(
  ...
  const observersEntries = Object.entries(observersOptions) as [U, IntersectionOptions][]
  const inViewsEntries = observersEntries
    .map<[U, InViewHookResponse]>(([observedName, observerOptions]) => [
      observedName,
      // eslint-disable-next-line react-hooks/rules-of-hooks
      useInView(observerOptions)
    ])
  const inViews = Object.fromEntries(inViewsEntries) as Record<U, InViewHookResponse>

  React.useEffect(() => {
    const sortedInViewsEntries = inViewsEntries.reverse()

    const lastInViewEntry = sortedInViewsEntries
      .find(([, [, inView]]) => inView)
  ...
  return [
    elementInView,
    inViews
  ] as [U | null, Record<U, InViewHookResponse>]
}